...
from smcon.model.model import TypeParser, DataModel, VarType, VariableModel, VarInfo, \
...
from smcon.ppt import PptTopLevel, PptType, TxType
...
from smcon.abstractType.AbstractType import MyContract
...
class smcon:
    ...
    def generate_trace_slice_invariants(self, inv_file = None):
        import os 
        if inv_file is None:
            inv_file = os.path.join(RESULT_DIR, self.address+"-"+self.contractName+"-trace.inv")
        output_format = "json"
        if output_format == "json":
            json_results = list()
