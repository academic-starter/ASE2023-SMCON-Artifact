...
from smcon.model.model import VariableModel, StructVariableModel, ArrayVariableModel, MappingVariableModel
import re  
...
class Event:
    ...
    def computeState(self, statevar):
        ...
        for parameter in self.getParameters():
            if parameter.getName() == pure_statevar if not isOrignal else parameter.getName() == "ori("+pure_statevar+")":
...
def covertTx2Event(tx: Transaction) -> Event:
    ...
    for item in tx.pre_state.variables:
        if isinstance(item, ArrayVariableModel):
            parameters.append(Parameter("ori("+item.varName+")", item.varValue, item.item_var_type))
        elif isinstance(item, MappingVariableModel):
            parameters.append(Parameter("ori("+item.varName+")", item.varValue, item.val_var_type))
        else:
            parameters.append(Parameter("ori("+item.varName+")", item.varValue))
    for item in tx.post_state.variables:
        if isinstance(item, ArrayVariableModel):
            parameters.append(Parameter(item.varName, item.varValue, item.item_var_type))
        elif isinstance(item, MappingVariableModel):
            parameters.append(Parameter(item.varName, item.varValue, item.val_var_type))
        else:
            parameters.append(Parameter(item.varName, item.varValue))
